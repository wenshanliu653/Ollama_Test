...
# add DATABASE_URL 這一行  OpenWebUI 會 改用 MySQL 來存儲自身的數據，而不會再使用 SQLite
#DATABASE_URL = "mysql+pymysql://root:12345678@localhost:3306/new_db"
...
# 手動加入 Python 模組搜尋路徑
sys.path.append("../")  # 修改為你的實際路徑


# 新增代碼
from OllamaSQLQueryTool import OllamaSQLQueryTool  # 確保這是你的 SQL 查詢工具的正確路徑
...
def execute_sql_query(question):
    """
    這個函數讓 OpenWebUI 透過 Ollama 執行 SQL 查詢
    """
    try:
...
# **手動加入 SQL 工具的搜尋路徑**
sys.path.append("/Users/sandyliu/sql_tool")  # 修改為你的 `your_sql_query_tool.py` 的實際路徑

# **導入你的 SQL 查詢工具**
try:
    from your_sql_query_tool import OllamaSQLQueryTool  # 確保 `your_sql_query_tool.py` 存在
except ModuleNotFoundError as e:
    print(f"❌ 無法載入 your_sql_query_tool: {e}")
    sys.exit(1)
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5001)  # 啟動 Flask API，監聽 5001 端口
'''    
